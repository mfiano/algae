...
(defpackage #:net.mfiano.lisp.algae.noise.open-simplex-3d
  (:local-nicknames
...
(in-package #:net.mfiano.lisp.algae.noise.open-simplex-3d)
...
(u:defun-inline make-state (sampler x y z)
...
(u:defun-inline extrapolate (sampler xsb ysb zsb dx dy dz)
  (let* ((table (the (simple-array u:ub8 (256)) (table sampler)))
         (index (c:pget table zsb ysb xsb)))
    (+ (* (aref +gradients+ index) dx)
       (* (aref +gradients+ (1+ index)) dy)
       (* (aref +gradients+ (+ index 2)) dz))))

(u:defun-inline contribute (state dx dy dz xsb ysb zsb)
  (let ((a (- 2 (* dx dx) (* dy dy) (* dz dz))))
    (when (plusp a)
      (incf (value state)
            (* (expt a 4)
               (extrapolate (sampler state) xsb ysb zsb dx dy dz))))
    (values)))
...
(defun contribute2 (state)
  (let ((xsb (xsb state))
    ...
    (contribute state (dx0 state) (dy0 state) (dz0 state) (1+ xsb) (1+ ysb)
                (1+ zsb))))
...
(defun contribute3 (state)
  (let ((xsb (xsb state))
    ...
    (contribute state (dx6 state) (dy6 state) (dz6 state) xsb (1+ ysb)
                (1+ zsb))))

(defun contribute4 (state)
  (contribute state (dx-ext0 state) (dy-ext0 state) (dz-ext0 state)
              (xsv-ext0 state) (ysv-ext0 state) (zsv-ext0 state))
  (contribute state (dx-ext1 state) (dy-ext1 state) (dz-ext1 state)
              (xsv-ext1 state) (ysv-ext1 state) (zsv-ext1 state)))
...
(defmethod c:make-sampler ((type (eql :open-simplex-3d)) seed)
  (declare (ignore seed))
  (let* ((table (rng:shuffle 'c::rng +permutation+))
